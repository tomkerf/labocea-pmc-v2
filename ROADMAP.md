...
|------|-------|-------------------|
| 2026-08-06 | Rapports ✅ | La liste des rapports rédigés est désormais repliée (collapsée) par défaut dans la page des Rapports, avec un toggle épuré de style Apple (ChevronDown animé et bouton discret). |
| 2026-07-26 | Dashboard ✅ | Refonte esthétique complète de l'onglet « Mon activité » (StatCard avec icônes Lucide, DonutChart animé, grille hero planning dominant, `ATraiterWidget` à onglets segmentés remplaçant 5 accordéons, `TodosWidget` en bande pleine largeur) puis 4 correctifs post-revue : bug bloquant d'affichage (état figé au premier rendu Firestore), contraste WCAG AA du token `--color-text-tertiary`, emojis résiduels → Lucide, motif ARIA Tabs complet avec navigation clavier. |
| 2026-07-20 | Rapports, Tâches & Messagerie ✅ | Refonte UI/UX complète en style pastel Apple-style : (1) page des Rapports (badges de délai pastel, groupes de clients épurés, inputs et boutons soignés), (2) page des Tâches (icônes Lucide à la place des émojis, badges de priorité et d'état en tons pastels, états vides élégants), (3) Messagerie (en-tête de chat, barre de suggestions @mentions et barre d'input des messages sans styles inline et bénéficiant du flou de fond natif backdrop-blur-md). |
| 2026-07-20 | Plan de charge, Vue annuelle & Demandes ✅ | Refonte du modèle de calcul (points de charge) et **refonte UI/UX de l'onglet Pilotage & page Demandes** : (1) page de charge allégée (en-tête collapsible, KPIs segmentés multicolores, heatmap pastelle), (2) vue annuelle modernisée (badges pastels, lignes pliables plus discrètes, pastilles de prélèvements épurées avec coches fines), (3) Kanban des Demandes clients modernisé (colonnes unifiées, cartes de demandes avec badges temporels pastels et icônes Lucide, zone archives restructurée). |
...
| 2026-07-05 | Messagerie d'équipe | Messagerie d'équipe temps réel avec mentions (@) et autocomplétion. Ajout des discussions privées individuelles (DMs, style WhatsApp) avec sidebar de canaux, recherche de contacts, et badges de non-lus par discussion. Sécurisation stricte des DMs dans `firestore.rules` par filtrage des participants autorisés. Ajout des sondages interactifs (Polls, style WhatsApp) avec barres de progression des votes, vote/dé-vote en un clic, et liste de votants résolus par trigramme. Intégration de l'App Badging API pour afficher de façon native le point rouge de notifications sur l'icône de l'app sur l'écran d'accueil iOS. Unification des pastilles de notification in-app au rouge iOS-style (PC et mobile) et modification dynamique du titre de l'onglet du navigateur (ex: (1) Labocea PMC). Alignement graphique de la pastille de messagerie sur celle de l'onglet Missions (rose clair avec texte rouge foncé). Ajout du partage de photos et d'images en temps réel (upload Firebase Storage, compression HEIC, visionneuse plein écran interactive). Ajustement de la hauteur du chat sur mobile pour éviter le chevauchement avec la barre de navigation du bas (BottomTabBar). Correction du scroll et padding global de la page de chat dans le layout parent (AppLayout) et bascule vers les unités DVH pour iOS. |
...
| 2026-06-29 | Estimation volume — simplification UX ✅ | Suite au retour « je ne comprends rien » : refonte en deux onglets Estimer (épuré, défaut) / Données (préparation), état vide guidé, libellés clairs Date/Pluie/Volume, import CSV en bouton visible. Réorga UI pure (0 modif logique/tests). Règles Firestore points-rejet déployées. Staging + push. |
| 2026-06-29 | Estimation volume — détection valeurs aberrantes ✅ | Détection par résidu > 2σ + interrupteur « Exclure les valeurs aberrantes » (R² avant/après affiché, points estompés sur le graphe). Garde-fous : max 20 % exclus, min 5 points restants. Conservateur par choix (n'aide pas sur un nuage sans tendance — honnêteté statistique). 5 tests, suite 168/168. Staging. |
...
| 2026-07-03 | Bugfix UI + SW cache ✅ | MissionsPage cartouche « Liste/Vue annuelle » s'étire pleine largeur. Root causes : (1) SW CACHE_VERSION hardcodée ne change pas = cache stale entre déploiements, (2) Tailwind v4 purge classes dans template literal conditionnel = flex/w-fit absent du CSS. Fixes : className statique + inline styles (display flex, width fit-content, alignSelf, marginLeft), CACHE_VERSION 'pmc-v2-2'→'pmc-v2-3'. Staging 495e2548→edac02b1, commit c3a37d5. Users : hard-refresh (Cmd+Shift+R) requis. À long-terme : injecter CACHE_VERSION dynamiquement (hash/timestamp). |
| 2026-07-03 | Rapport d'intervention prévu — modale tournée ✅ | Checkbox « Rapport d'intervention prévu » ajoutée dans SaisieRapideModal (visible si statut Réalisé), pré-remplie depuis le sampling. TourneePage écrit `rapportPrevu` + calcule `rapportDatePrevue` (doneDate + 1 mois) via nouvel utilitaire partagé `computeRapportDatePrevue()` (lib/samplings.ts). 261/261 tests, lint 0. Commit `215fd45`, staging `bc0f4c19`. Non testé visuellement (pas d'accès identifiants) — à valider par Tom. |
| 2026-07-03 | Investigation Sentry — faux positif Vite/dev ✅ | Issue `LABOCEA-PMC-V2-5` (« send was called before connect ») analysée : stack 100% dans `/@vite/client`, tags `environment: development` + `url: localhost:5173` confirment une erreur de dev local (Firestore Listen stream transitoire → console.warn → interception Sentry → relai Vite HMR WS non connecté). Aucun impact prod, aucune modif de code. Reste à décider : filtrer ce type d'erreur côté config Sentry. |
| 2026-07-03 | Dédup + Sentry dev + réorg Planning/Missions + filtres Charge ✅ | Dédup `computeRapportDatePrevue` (usePlanActions/SamplingForm) + Sentry désactivé en dev (fix du faux positif). Onglet Année retiré du Planning, vue Charge déplacée en 3ᵉ onglet de MissionsPage (avec Liste/Vue annuelle). Filtres Site (Brest/Quimper) + Technicien ajoutés sur l'onglet Charge, câblés jusqu'à YearMatrixView/WorkloadMatrixView, exécutés en subagent-driven-development (4 tâches + review indépendant par tâche). 261/261 tests, commits `cdca2a2`→`14ae250`. |
| 2026-07-03 | Fix filtre Site MissionsPage ✅ | Vérification visuelle staging (Chrome MCP) : filtre Technicien OK, mais filtre Site toujours vide malgré 6 préleveurs configurés (Brest/Quimper) en Admin. Cause : `MissionsPage` lisait `usePreleveursStore` sans jamais monter `usePreleveursListener()` (listener local par page, seule `PlanningPage` le montait) — store vide hors navigation préalable par le Planning. Fix : listener ajouté dans `MissionsPage.tsx`. Revalidé sur staging après déploiement : Brest→0 (cohérent, aucun client Brest en staging), Quimper→567/568. Commit `60589a1`. |
| 2026-07-04 | RUN_BOOK enrichi + campagne de tests mutations Firestore ✅ | RUN_BOOK : troubleshooting 5→13 entrées (incidents réels DEV_LOG), nouvelle section Logs/Debugging (Sentry, Firebase Console, wrangler tail), risque Firestore partagé 🟡→🔴. Tests : `useDocumentData` (auto-save/debounce/conflits), `clientService`/`equipementService` (garde-fou `runTransaction`), `usePlanActions` (audit trail, doneBy, rapport prévu), `usePlanningActions` (report, retrait, verrou anti-double-clic). **Bug de prod corrigé** : `computeRapportDatePrevue` décalait d'un jour au passage à l'heure d'été (arithmétique UTC vs locale). Suite 261→313 tests. Commits `211c38e`→`01d9675`. |
...
| 2026-07-19 | Refonte Mode d'emploi ✅ | Refonte de la page d'aide (/aide) en format interactif et fluide Apple-style (split-pane sur desktop, accordéons animés sur mobile). Recherche globale par mots-clés, barre de progression persistée, FAQ raccourcis et raccourci clavier. 339 tests OK. |
| 2026-07-20 | Refonte UI/UX premium Apple-style ✅ | Refonte globale des modules. Harmonisation Sidebar/AppLayout/Tabbar, polices et couleurs KPI d'origine restaurées, correction de règle ESLint bloquante sur la page d'aide, compactage de l'en-tête du planning et restauration du layout original de la vue annuelle (p-4 md:p-6, rounded-xl). |
| 2026-07-20 | Revue UI/UX Rapports/Tâches + audit sécurité complet ✅ | Confirmation passage Firebase Blaze (plus de risque quota Spark à l'adoption Brest). 2 bugs UX corrigés (hover manquant RapportRow, actions Modifier/Supprimer invisibles au tactile dans TodoRow). Audit sécurité complet : fallback dangereux clé Firebase→Gemini retiré, clé Gemini perso migrée localStorage→sessionStorage. 353/353 tests, commits `4a65702`→`7c9d546`. |
| 2026-07-21 | Sentry & UI Actualités ✅ | Amélioration UI/typographie des actualités du tableau de bord (renderMarkdownSnippet + tailles text-sm/text-xs). Corrections robustesse Sentry : modale d'export (preleveurs optionnels), en-tête HTTP CSP Sentry, tris YearMatrixView (siteNom/nom optionnels) et boucles samplings (null-safety). |
| 2026-07-21/22 | Pilotage — drill-down mensuel, export PDF, polish UI ✅ | Fix page Pilotage en erreur (Worker Cloudflare pas à jour, redéploiement). Vue annuelle : clic sur un mois isole la colonne (grisage) et une loupe dédiée ouvre la liste des prélèvements du mois (tous statuts). Export PDF (aperçu + impression) de cette liste, réutilisant le pattern reportHtml/PdfPreviewModal existant. Polish : page compactée (marges, filtres sur 1 ligne avec labels sr-only, toggle Vue annuelle/Charge allégé, en-tête de tableau resserré). |
| 2026-07-28 | Fréquence hebdo, heure planification, plein écran, sidebar rail, padding ✅ | Merge fréquence hebdomadaire (13 commits). Heure optionnelle + date lisible dans la modale « Planifier le » (plannedTime). Mode plein écran YearMatrixView (overlay in-app, actif Planning + Pilotage). Sidebar repliable en rail d'icônes 64px (alternative retenue au dock flottant proposé par Gemini — 19 destinations/3 badges ne tiennent pas dans un dock). Harmonisation padding/largeur des 25 pages en 3 paliers (documenté design-system.md). Housekeeping : suite refonte dashboard + PCCD commités, doublons DEV_LOG supprimés, 4 worktrees nettoyées, 42 commits poussés. 437/437 tests verts. |
| 2026-07-28 | Centrage dashboard + ratio hero grid 🟡 | Fix `mx-auto` manquant sur le conteneur dashboard (centrage). Ratio hero grid Planning/rail élargi 1.55fr→2.2fr en faveur du planning. Bump `CACHE_VERSION` du Service Worker. Non résolu en fin de session : utilisateur ne voit toujours pas le nouveau ratio malgré un déploiement staging vérifié correct côté serveur — suspicion cache navigateur/SW persistant à investiguer (Unregister manuel, navigation privée). |
| 2026-07-29 | Bugfixes visite préliminaire (save/photo/point) ✅ | Bouton Enregistrer et ajout photo silencieusement bloqués par une validation sans retour visuel (technicienNom à espace vide, point sans nom) : messages d'aide + toasts ajoutés. Compression automatique des photos >10 Mo avant upload (4 points d'upload concernés). "Créer un point de prélèvement" ajouté en récap de fin de formulaire + bug de lien orphelin corrigé (pointMesureId pointant vers un point supprimé masquait le bouton à tort). Dashboard : teaser et compaction testés puis annulés à la demande, retour à l'état d'origine. Commit `0a16f35` poussé, 437/437 tests. Plan d'isolation Firestore staging/prod redemandé et re-présenté (exécution reportée, voir plans/oui-encapsulated-taco.md). |
| 2026-07-29 | Changelog figé, alerte matériel Bilan 24h, dette technique ✅ | "Nouveautés" ne se mettait jamais à jour (liste statique `changelog.ts`, figée depuis le 21/07) — 9 versions ajoutées (170→178) résumant le shipping récent. Nouvelle alerte non bloquante (toast `warning`, 4e type) quand un Bilan 24h est planifié sans préleveur/débitmètre disponible ce jour-là (J1+J2), câblée dans `usePlanningActions`. Audit react-doctor (40→41/100) : 1 seul vrai bug trouvé et corrigé (`Sidebar.tsx`, effet de bord dans un updater `setState`), le reste déjà FP documentés. Backlog priorisé sauvegardé en mémoire (contraste WCAG, tests E2E, bundle size). 445/445 tests. |
| 2026-07-30 | Badge J1/J2 dashboard, fix Google Maps, ménage planning ✅ | Badge J1/J2 ajouté sur les cartes du widget "Planning du jour" du Dashboard pour les Bilans 24h (même style que la vue Planning complète). Bug corrigé : carte Google Maps affichait une icône cassée sur la page Détail mission — `sandbox` de l'iframe manquait `allow-same-origin`, empêchant les scripts Maps de s'exécuter (2 composants concernés). Bandeau "Astuce — glisse sur plusieurs jours…" retiré du Planning à la demande de Tom (composant, état et clé localStorage nettoyés). 445/445 tests. |
| 2026-07-30 | Fix + refactoring badge J1/J2 bilan 24h ✅ | Badge affichait J1 à tort sur un bilan 24h "Réalisé" (devrait être J2) — logique dupliquée et incohérente entre 3 fichiers. Centralisation dans une fonction unique `getBilan24Dates()` (`planningUtils.ts`), reprise par `usePlanningData.ts` et `useDashboardStats.ts`. 8 tests de régression ajoutés, 453/453 tests. |
| 2026-07-31 | Coche animée, max-width harmonisé, bilan mensuel missions ✅ | Point→coche animée (spring pop) quand un prélèvement passe "réalisé", sur le Dashboard et la vue Planning. Dashboard aligné sur `max-w-4xl` comme le reste de l'app (audit des 19 onglets, 3 paliers cohérents). Nouvelle page `/missions/bilan` (fait/non fait du mois, bascule Mes missions/Équipe) accessible depuis la carte KPI "Missions ce mois" (Dashboard + Suivi équipe, nouvelle 5ᵉ carte). 453/453 tests. |
| 2026-08-01 | Bug page blanche (root cause) + ratio hero grid + largeurs harmonisées ✅ | Root cause du bug "page blanche" enfin identifiée après 4 correctifs infructueux : tout le contenu de page dépendait d'une animation framer-motion pilotée par rAF, gelée sur onglet en arrière-plan → `opacity:0` figé indéfiniment. Fix fail-visible : animation CSS pure sans fill-mode (AppLayout + DashboardPage). Ratio hero grid Dashboard corrigé (`minmax(0,Nfr)` manquant expliquait l'absence d'effet des tentatives précédentes) et réglé au nombre d'or (1,618:1). Missions, Matériel, Métrologie, Maintenances alignés sur `max-w-4xl`. 453/453 tests. |
| 2026-08-05 | Audit sécurité firestore.rules + tests émulateur ✅ | Audit ciblé des règles Firestore (skill `tester-code`, Opus) : 3 bugs critiques identifiés (usurpation d'identité chat, `delete` admin contournable via `update`, aucun test de règles possible). 3 correctifs déployés en prod (`senderUid` dfc037f, `immutableOn` élargi clients-v2/todos ed82cdd, `selfOnlyKeyDiff` anti-duplication votes/réactions 4d5c183 — piège limite CEL 1000 expressions rencontré et contourné). Infra de test débloquée : Java installé (Homebrew), `@firebase/rules-unit-testing`, `npm run test:rules` (10 tests). Reste : isolation staging/prod, migration samplings. |
| 2026-08-05 | Tests E2E débloqués + bug sauvegarde perdue corrigé ✅ | Isolation staging/prod reportée (plan prêt, accès vérifiés). Suite E2E Playwright enfin fonctionnelle (3 fixes infra : override jose@4, Chromium installé, modale changelog neutralisée au login ; `e2e/**` exclu de vitest.config.ts). `prelevement.spec.ts` a exposé un vrai bug : "Terminer la mission" perdait sa sauvegarde silencieusement si la page était atteinte par deep-link (notif push) — `navigate(-1)` sans historique SPA tuait le debounce 800ms avant qu'il parte. Fix : `saveNow()` ajouté à useClientData (écriture immédiate attendue), utilisé par Terminer/Non effectué/Décaler. 463/463 tests, déployé staging. Découverte annexe non corrigée : fixtures E2E partagées entre specs (tournee.spec.ts dépend de l'ordre d'exécution). |
| 2026-08-06 | Suite audit firestore.rules : createdAt, technicienUid, storage delete ✅ | 3 correctifs de sécurité déployés en prod : `createdAt == request.time` sur chat-messages (5b8a763, empêche un message antidaté), `technicienUid` immuable sur visites (7bd7a51, vérifié qu'aucun flux UI ne le réassigne), storage.rules delete restreint à l'auteur/admin via customMetadata.uploadedBy posé à l'upload + firestore.get() cross-service (e11570b, 4 points d'appel uploadPhoto.ts modifiés, fallback pour photos pré-existantes). Points 2/9 (delete Firestore contournable + concurrence — même cause racine, samplings imbriqué dans clients-v2) reportés délibérément : chantier de migration en sous-collection, ampleur comparable à l'isolation staging/prod. Trou de config vitest corrigé au passage (projet 'unit' scannait aussi firestore-rules/**, masqué par un émulateur résiduel). 453/453 tests, E2E 3/3 stables, déployé staging. |
| 2026-08-06 | Bug prod planification + chasse aux bugs autonome ✅ | Bug signalé par Tom : planifier un créneau sans heure échouait systématiquement depuis ~10 jours (`plannedTime: time \|\| undefined` → le SDK Firestore refuse une clé valant undefined). Reproduit contre l'émulateur après une première hypothèse fausse déployée à tort. Fix + `ignoreUndefinedProperties` global (45228dd). Bouton "Créer" grisé sans explication sur Infos terrain : `validationHint()` unique source de la validation et du message (260ca39). Chasse autonome demandée par Tom : chantier 1 (37 `catch` audités → vrai bug trouvé, suppression de photo qui semblait réussir alors qu'elle échouait depuis le durcissement storage.rules du matin ; ajout de `reportError()` car Sentry ne capture pas les erreurs interceptées — 4841cee) et chantier 2 (13 boutons `disabled` audités, 2 corrigés — be19289). Chantiers 3 (E2E) et 4 reportés. 461 tests, E2E 3/3. |
| 2026-08-06 | Changelog figé rattrapé + rituel fin-session ✅ | "Nouveautés" figé à v178 (29 juillet) depuis 7 sessions, signalé par Tom — déjà arrivé une fois (session 196). 5 entrées ajoutées (179→183) résumant tout le shipping utilisateur-visible depuis (alerte matériel Bilan 24h, badge J1/J2, fix carte Google Maps, coche animée, bilan mensuel missions, fix page blanche définitif, ratio hero grid, harmonisation largeurs, fix "Terminer la mission", durcissement sécurité). Fix structurel : le skill local fin-session intègre désormais la mise à jour du changelog comme étape systématique (si impact visible utilisateur), pour éviter la récidive. |

| 2026-08-07 | CI/CD : diagnostic échecs déploiement, relance manuelle, mise à jour actions ✅ | 3 runs `Deploy Staging` rouges dus à une indisponibilité temporaire des runners GitHub (aucune étape démarrée), pas une régression build. Découverte annexe : dernier commit `d4e0634` n'avait déclenché aucun run CI (déployé manuellement la veille en local). Ajout de `workflow_dispatch` à `deploy-staging.yml` pour relancer sans commit vide (0d4ae99). Bump `actions/checkout@v5`, `setup-node@v5`, `wrangler-action@v4` pour éliminer l'avertissement de dépréciation Node.js 20 (5838b52). Aucun changement applicatif. |
