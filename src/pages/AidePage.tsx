...
export default function AidePage() {
  ...
  const visibleKeys = SECTIONS.filter(s => filter === 'tous' || s.roles.includes(filter)).map(s => s.key)
  const readVisibleCount = readSections.filter(k => visibleKeys.includes(k)).length
    ...
    <div className="max-w-6xl mx-auto px-4 py-8 flex flex-col gap-6">
      ...
      <div className="flex flex-wrap items-center gap-2 text-xs">
        ...
        {FAQS.map(faq => (
          <button
            key={faq.sectionKey}
            onClick={() => handleShortcutClick(faq.sectionKey)}
      ...
      <div className="grid grid-cols-1 md:grid-cols-[280px_1fr] gap-8 items-start">
        ...
        {/* Colonne de gauche (Navigation / Sidebar) */}
        <div className="flex flex-col gap-6 md:sticky md:top-6">
          
          {/* Sélecteur de rôle / profil */}
          <div className="rounded-xl p-4 bg-[var(--color-bg-secondary)] border border-[var(--color-border-subtle)] shadow-[var(--shadow-card)]">
            ...
            {filter !== 'tous' && filter !== defaultFilter && (
              <p className="text-xs mt-3 pt-3 border-t border-[var(--color-border-subtle)] text-[var(--color-text-tertiary)]">
                ...
                <button type="button" onClick={() => { setFilter(defaultFilter); setActiveSectionKey('debut'); }}
          ...
          <div className="rounded-xl p-4 bg-[var(--color-bg-secondary)] border border-[var(--color-border-subtle)] shadow-[var(--shadow-card)] flex flex-col gap-2">
            ...
            <div className="w-full h-1.5 rounded-full bg-[var(--color-bg-tertiary)] overflow-hidden">
              <motion.div
                ...
                style={{ background: progressPercent === 100 ? COLORS.SUCCESS : COLORS.ACCENT }}
              />
            </div>
          </div>

          {/* Liste des sections (Desktop uniquement) */}
              ...
              <div key={cat} className="flex flex-col gap-1">
                ...
                {filteredSections
                  ...
                  .map(sec => {
                    ...
                    const isRead = readSections.includes(sec.key)
                    const Icon = sec.icon
                    return (
                      <button
                        key={sec.key}
        ...
        {/* Colonne de droite / Détail ou Liste Mobile */}
        <div className="flex flex-col gap-4">
          ...
          {filteredSections.length === 0 ? (
            ...
            <>
              {/* VUE DESKTOP : Panneau de lecture centré */}
                  ...
                  {activeSection && (
                    ...
                    >
                      ...
                      <div className="px-6 py-5 border-b border-[var(--color-border-subtle)] bg-[var(--color-bg-tertiary)]/30 flex justify-between items-start gap-4">
                        ...
                        <button
                          onClick={() => toggleRead(activeSection.key)}
                          className="px-3 py-1.5 rounded-full text-xs font-semibold flex items-center gap-1.5 transition-all cursor-pointer"
                          style={{
                            background: readSections.includes(activeSection.key) ? 'var(--color-success-light)' : 'transparent',
                            color: readSections.includes(activeSection.key) ? COLORS.SUCCESS : COLORS.TEXT_SECONDARY,
                            border: `1px solid ${readSections.includes(activeSection.key) ? 'transparent' : 'var(--color-border)'}`,
                          }}
                        >
                          {readSections.includes(activeSection.key) ? (
                            <>
                      ...
                      {/* Pied de section : Navigation continu */}
                      <div className="px-6 py-4 border-t border-[var(--color-border-subtle)] bg-[var(--color-bg-tertiary)]/10 flex items-center justify-between gap-4">
                        ...
                        >
                          <span className={`size-4 rounded-full border flex items-center justify-center transition-colors ${readSections.includes(activeSection.key) ? 'bg-[var(--color-success)] border-transparent text-white' : 'border-[var(--color-border)]'}`}>
                            {readSections.includes(activeSection.key) && <Check className="size-2.5" />}
                          </span>
                          {readSections.includes(activeSection.key) ? 'Marquée comme lue' : 'Marquer cette section comme lue'}
                        </button>

                        {nextSection && (
                          <button
                            onClick={() => setActiveSectionKey(nextSection.key)}
