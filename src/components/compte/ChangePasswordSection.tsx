...
type PasswordFormAction =
  | { type: 'field'; field: keyof PasswordFormState; value: string }
  | { type: 'reset' }

const initialPasswordForm: PasswordFormState = { current: '', next: '', confirm: '' }
...
function passwordFormReducer(state: PasswordFormState, action: PasswordFormAction): PasswordFormState {
  switch (action.type) {
    ...
    case 'reset': return initialPasswordForm
  }
}

export function ChangePasswordSection({ email }: { email: string }) {
  const [open,     setOpen]     = useState(false)
  const [fields,   dispatch]    = useReducer(passwordFormReducer, initialPasswordForm)
  const [status,   setStatus]   = useState<'idle' | 'saving' | 'success' | 'error'>('idle')
  const [errorMsg, setErrorMsg] = useState('')

  async function handleSubmit(e: React.FormEvent) {
    e.preventDefault()
    if (fields.next !== fields.confirm) { setErrorMsg('Les mots de passe ne correspondent pas.'); setStatus('error'); return }
    if (fields.next.length < 6)  { setErrorMsg('Le mot de passe doit faire au moins 6 caractères.'); setStatus('error'); return }
    ...
    try {
      ...
      await reauthenticateWithCredential(user, EmailAuthProvider.credential(email, fields.current))
      await updatePassword(user, fields.next)
      setStatus('success')
      dispatch({ type: 'reset' })
      ...
      {open && (
        <form onSubmit={handleSubmit} className="px-5 pb-5 flex flex-col gap-3"
          ...
          <div className="pt-4 flex flex-col gap-3">
            {([
              { label: 'Mot de passe actuel', field: 'current' },
              { label: 'Nouveau mot de passe', field: 'next'   },
              { label: 'Confirmer',            field: 'confirm' },
            ] as { label: string; field: keyof PasswordFormState }[]).map(({ label, field }) => {
              ...
              return (
                <div key={label}>
                  ...
                  <input id={id} type="password" value={fields[field]} onChange={e => dispatch({ type: 'field', field, value: e.target.value })} required
