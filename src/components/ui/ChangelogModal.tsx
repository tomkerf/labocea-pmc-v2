      ...
      {entries.map(entry => (
        <div key={entry.version}>
          <div className="flex items-baseline gap-2 mb-2">
            <span className="text-xs font-semibold" style={{ color: COLORS.TEXT_SECONDARY }}>
          ...
          <ul className="flex flex-col gap-1.5">
            {entry.items.map((item) => (
              <li key={item.label} className="flex items-start gap-2 text-sm" style={{ color: COLORS.TEXT_PRIMARY }}>
...
export default function ChangelogModal({ open, onClose }: { open: boolean; onClose: () => void }) {
  return (
    <BaseModal
      ...
      onClose={onClose}
      title="Nouveautés"
      icon={<Sparkles size={18} strokeWidth={1.8} style={{ color: COLORS.ACCENT }} />}
      footer={
        <button
          type="button"
          onClick={onClose}
